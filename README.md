...
* Uses [uv](https://github.com/astral-sh/uv)
  ...
  * [ruff](https://github.com/charliermarsh/ruff) for code formatting and
    quality analysis
...
1. Set next version to `BUMPED_VERSION`:
   This ensures that the `git-cliff --bumped-version` command produces
   consistent results.
...
1. Generate `CHANGELOG.md` and `RELEASE.md`:
   The script creates or updates the changelog and release notes using
   the bumped version:
...
1. Commit updated `CHANGELOG.md` and `RELEASE.md` then add tags and push:
   It commits the updated files, creates a tag for the new version,
